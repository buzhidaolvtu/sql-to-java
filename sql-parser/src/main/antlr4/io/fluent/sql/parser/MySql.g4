...
table_option:
    ...
    | MAX_ROWS EQ? value
    | MIN_ROWS EQ? value
    | PACK_KEYS EQ? (INTEGER_VALUE | DEFAULT)
    | PASSWORD EQ? STRING
    | ROW_FORMAT EQ? (DEFAULT|DYNAMIC|FIXED|COMPRESSED|REDUNDANT|COMPACT)
    | STATS_AUTO_RECALC EQ? (DEFAULT|INTEGER_VALUE)
    | STATS_PERSISTENT EQ? (DEFAULT|INTEGER_VALUE)
